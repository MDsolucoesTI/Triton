...
If Lastrec()<1
   Append Blank
   Repl codigo    With 1
   Repl descricao With 'Transferˆncia de Saldo'
   Unlock
   Append Blank
   Repl codigo    With 10
...
*      DBox('ATEN€ŽO !!!|Esta ‚ uma vers„o demonstra‡„o v lida at‚ '+DtoC(DtFimCont)+'|para maiores informa‡”es consulte:|Denny Paulista Azevedo Filho|Rua Rebou‡as de Carvalho, 46|Taubat‚ - SP|TelFax (0xx12) 221-5862.',,,0,.T.,,)
*   Else
*      DBOX('ATEN€ŽO !!!|A Data que foi fornecida n„o ‚ v lida|sua altoriza‡„o de uso vai at‚ '+DtoC(Paramet->DtFimCont)+'.|Qualquer d£vida entre em contato com sua empresa de suporte|ou diretamente com Denny Paulista Azevedo Filho',,,0,.T.,,)
*   EndIf
*   mViola++
*   mOrdem=.F.
*   If mViola>3
...
Do Case
   Case Vl=1
      XXX=MTab([SIM|NŽO],[Dar baixa nesta conta a pagar ?])
   Case Vl=2
      XXX=MTab([SIM|NŽO],[Dar baixa nesta conta a receber ?])
   Case Vl=3
      XXX=MTab([SIM|NŽO],[Estornar esta conta a pagar ?])
   Case Vl=4
      XXX=MTab([SIM|NŽO],[Estornar esta conta a receber ?])
   Case Vl=5
      XXX=MTab([SIM|NŽO],[Deletar esta conta ?])
EndCase
Keyboard Chr(13)
Return(.T.)
