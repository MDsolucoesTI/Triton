...
SETCOLOR(drvtitbox)
CAIXA(mold,14,32,23,77)                // monta caixa da tela
SETCOLOR(drvcorbox)
@ 15,33 SAY "          T R I T O N - VersÑo 2.1"
@ 16,33 SAY "            Controle Imobili†rio"
@ 17,33 SAY "         ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ"
@ 18,33 SAY "     SOLUTION - Consultoria & Sistemas"
@ 19,33 SAY "ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ"
@ 22,33 SAY "         ,             -"
@ 22,44 SAY DATE()
@ 22,35 SAY NSEM(DATE())
IF imp_cp                              // imprime campos do paramentro
   @ 20,37 SAY M->empresa
   @ 22,59 SAY M->operador PICT ""
ENDI
...
IF dr_v>0                                               // escolheu um...
   ...
   FOR t=1 TO nss                                       // faz backup de todos os arquivos
               ...
               ELSE
                  FOR i=1 TO nss                        // mata do disquete todos os
                     ...
                     IF FILE(db) .AND. db!=dr_v+arqdbf  // menos o arquivo atual
                        ERASE (db)
                     ENDI
                     db=dr_v+sistema[i,O_ARQUI]+".dbt"  // idem para o .dbt
      ...
      IF !cn                                            // se o backup nao foi cancelado,
         ...
         IF testdb_<2.AND.FILE(drvdbf+sistema[t,O_ARQUI]+".dbt")
            testdb_=2
            t--
         ELSE

            #ifdef COM_REDE
               IF testdb_<3.AND.FILE(drvdbf+LEFT(sistema[t,O_ARQUI],3)+"_SEQ.dbf")
                  testdb_=3
                  t--
               ELSE
                  testdb_=1
   ...
   FOR t=1 TO nss                                              // restaura backup de todos arquivos
      ...
      IF !cn                                                   // se operacao nao foi cancelada
         ...
         IF t!=disco_arq .OR. disco_qtd=disco                  // foi o ultimo arq copiado pelo backup?
            FCLOSE(destino)                                    // fecha arquivo destino
            IF testdb_<2.AND.FILE(dr_v+sistema[t,O_ARQUI]+".dbt")
               testdb_=2
               t--
            ELSE

               #ifdef COM_REDE
                  IF testdb_<3.AND.FILE(dr_v+LEFT(sistema[t,O_ARQUI],3)+"_SEQ.dbf")
                     testdb_=3
                     t--
                  ELSE
                     testdb_=1
...
DO WHIL op_a>0
   ...
   op_a=DBOX(SUBS(msg,2),03,69,E_MENU,NAO_APAGA,,,,op_a)
   IF op_a>0                                              // escolheu um...
      op_sis=op_a
      ii=VAL(SUBS(db,op_sis*2-1,2))                       // acha sua subscricao
      FOR op_sis=ii TO nss+1                              // indexa "pais e filhos"
...
DO WHIL .t.
   ...
   cod_sos=42
   op_imp=DBOX(rel_imp,05,77,E_MENU,NAO_APAGA,,,,op_imp)
...
DO WHILE .t.
   cod_sos=44
   msg="Pano do fundo|Janelas|Caixas de di†logos|Avisos e erros|Menus e campos|Tela de digitaáÑo|Tela de digitaáÑo nÑo ativa|Janela de ajuda"
...
PROC PARAMETROS(cp_,fr_)  // Atualiza arquivo de parÉmetros da aplicaáÑo
LOCAL ar_:=SELECT(), i_
SELE 0

#ifdef COM_REDE
   USEARQ(dbfparam,.t.,,,.f.)  // abre arquivo de parametro sem indice
#else
   USE (dbfparam)              // abre arquivo de parametro
#endi
...
LOCAL op_sis, ind_ok, ind_an, arqtmp, chv_, p_ , x_, ind01, ind02
op_sis=EVAL(qualsis,ALIAS())                         // subscricao do vetor sistema
...
FOR op_sis=op_sy_ TO nss                       // a partir do "pai" pega os "filhos"
   IF op_s!=op_sis                             // menos o arquivo atual
      IF op_sis>op_sy_                         // pode ser um "filho"
         IF PEGAPAI(op_sis)!=op_sy_            // se nao tem o mesmo "pai"
            EXIT                               // cai fora...
         ENDI
      ENDI

      #ifdef COM_REDE
         IF !USEARQ(sistema[op_sis,O_ARQUI],.f.,20,1)
            RETU                               // nao conseguiu abrir o arquivo
         ENDI
      #else
         USEARQ(sistema[op_sis,O_ARQUI])       // abre o arquivo
      #endi
      ...
      i_=LEFT(sistema[op_sis,O_ARQUI],3)
...
PRIV  chv_1, chv_2, tem_borda, op_sis:=op_sis, chv_rela:="", chv_1:="", chv_2:=""
edic=1
t_w:=SETKEY(K_CTRL_W,NIL)                               // desabilita e salva
t_r:=SETKEY(K_CTRL_R,NIL)                               // as teclas de controle
t_c:=SETKEY(K_CTRL_C,NIL)
...
DO WHIL .t.
   ...
   IF pri_vez
      ...
      #ifdef COM_REDE
         IF !USEARQ(sistema[op_sis,O_ARQUI],.f.,20,1)  // se nao abriu arquivo,
            RETU                                       // volta
         ENDI
      #else
         USEARQ(sistema[op_sis,O_ARQUI])               // abre DBF/NTX
      #endi
      ...
      IF LEN(sistema[op_sis,O_DBRELA])>0.OR.;         // se tem dbfs relacionados
         ...
         IF ii>0
            ...
            ELSE                                      // F8...
               ...
               ELSE
                  op_sis=IF(op_sis=pai_sis,op_sis+1,pai_sis)
               ENDI
            ENDI
         ENDI
         cr_="cr_"+ALIAS()
...
DO WHILE LEN(sistema[op_sy_,O_CPRELA])>0      // se existir um pai...
   ...
   NEXT
   PTAB(SUBS(chv_rela,2),db_,1)               // abre/relaciona o pai
...
IF op_o>0
   ...
   IF LEN(c_ampo)>0 .OR. op_o!=3                                    // existe processo
      ...
      ELSE
         ...
         op_a=2
         IF op_o=1 .AND. !EMPT(sistema[op_sis,O_CONDEXC,1])
            ALERTA()
            msg="Obedecer condiáîes|Ignorar condiáîes"
            msgt="|EXISTEM CONDIÄôES ESPECIAIS|PARA ATUALIZACAO DESTE ARQUIVO"
            op_a=DBOX(msg,,,E_MENU,,"ATENÄéO, "+usuario+msgt)
            IF op_a=0
         ...
         IF LEN(sistema[op_sis,O_DBRELA])>0                         // se tem filho
            IMPRELA()                                               // vamos mostra-lo
         ENDI
         &getsx.()                                                  // conteudo do registro
...
#ifdef COM_REDE
   IF !USEARQ(sistema[op_sis,O_ARQUI],.f.,20,1)         // se nao abriu arquivo, volta
      RETU
   ENDI
#else
   USEARQ(sistema[op_sis,O_ARQUI])                      // abre DBF/NTX
...
FOR i_=1 TO LEN(sistema[op_sy_,O_DBRELA])                // para cada "filho" do arquivo
   ...
   ind_ord=INDEXORD(1)                                   // indice principal
   ...
   DO WHILE &(INDEXKEY(1))=chv_
      ...
      SKIP                                               // proximo...
   ENDD
   INDEXORD(ind_ord)                                     // volta ao indice em uso e
...
IF op_menu=INCLUSAO                                 // teclas disponiveis na
   ...
   MsgCen="[F3] Repetir, [F4] Confirma,[F9] Outros Arquivos"
